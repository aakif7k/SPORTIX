...
export const AthleteProfile: React.FC = () => {
  ...
  return (
    <article className="max-w-4xl mx-auto space-y-6 pb-20 text-text-primary">
      ...
      <div className="relative rounded-3xl overflow-hidden bg-surface border border-border-muted shadow-2xl">
        {/* Cover Photo */}
        <div className="h-44 sm:h-60 relative overflow-hidden bg-gradient-to-r from-surface via-elevated to-surface">
          <img 
            ...
            <div className="flex flex-col sm:flex-row items-center sm:items-end gap-4 text-center sm:text-left">
              <div className="relative">
                <img 
                  ...
                  className="w-28 h-28 sm:w-32 sm:h-32 rounded-3xl object-cover ring-4 ring-surface shadow-2xl bg-elevated border border-border-muted"
                />
                <span className="absolute bottom-1 right-1 w-5 h-5 rounded-full bg-accent ring-4 ring-surface flex items-center justify-center text-volt-text">
              ...
              <div className="space-y-1">
                ...
                <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2 font-mono text-xs text-text-secondary">
                  <span className="text-accent font-bold">@{profileUser.username || 'athlete'}</span>
                  <span>•</span>
            ...
            <div className="flex items-center justify-center gap-3 w-full sm:w-auto">
              {isMe ? (
                <button
                  ...
                  className="w-full sm:w-auto px-5 py-2.5 rounded-xl bg-accent hover:bg-accent/90 text-volt-text font-mono font-bold text-xs uppercase tracking-wider transition-all flex items-center justify-center gap-2 shadow-md"
                >
                ...
                <>
                  <button
                    ...
                    className={`px-4 py-2.5 rounded-xl font-mono text-xs font-bold uppercase tracking-wider transition-all flex items-center gap-2 ${
                      isConnected 
                        ...
                        : 'bg-accent text-volt-text hover:bg-accent/90'
                    }`}
                  >
                  ...
                  <button
                    ...
                    className="px-4 py-2.5 rounded-xl bg-elevated border border-border-muted hover:border-accent text-text-primary font-mono text-xs font-bold uppercase tracking-wider transition-all flex items-center gap-2"
                  >
      ...
      {isMe && (
        <div className="p-4 rounded-2xl bg-surface border border-border-muted flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-sans font-bold text-xs text-text-primary">Open for Squad Scouting & Matches</p>
        ...
        >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="md:col-span-2 space-y-6">
                <div className="p-6 rounded-3xl bg-surface border border-border-muted space-y-4">
                  <h2 className="font-sans font-bold text-sm text-text-primary uppercase tracking-wider flex items-center gap-2">
                ...
                <div className="p-6 rounded-3xl bg-surface border border-border-muted space-y-4">
                  <h2 className="font-sans font-bold text-sm text-text-primary uppercase tracking-wider flex items-center gap-2">
                    <TrendingUp size={16} className="text-accent" /> PlayerDNA Attribute Distribution
                  </h2>
                  <div className="h-64 w-full">
                    <ResponsiveContainer width="100%" height="100%">
                      <RadarChart data={radarData}>
                        <PolarGrid stroke="var(--border)" />
                        <PolarAngleAxis dataKey="subject" tick={{ fill: 'var(--text-secondary)', fontSize: 11, fontFamily: 'Urbanist' }} />
                        <Radar name={profileUser.name} dataKey="A" stroke="var(--accent)" fill="var(--accent)" fillOpacity={0.25} />
                      </RadarChart>
                    </ResponsiveContainer>
                  </div>
                </div>
              ...
              <div className="space-y-6">
                <div className="p-6 rounded-3xl bg-surface border border-border-muted space-y-4">
                  <h2 className="font-sans font-bold text-sm text-text-primary uppercase tracking-wider flex items-center gap-2">
                    <Shield size={16} className="text-accent" /> Active Squad
                  </h2>
                  {userSquad ? (
                    <div className="p-4 rounded-2xl bg-elevated border border-border-muted space-y-3">
                      <div className="flex items-center justify-between">
                        <span className="font-sans font-bold text-sm text-text-primary">{userSquad.name}</span>
                        <span className="font-mono text-[10px] text-accent font-bold px-2 py-0.5 rounded-full bg-accent/10">
                          {userSquad.sport}
                        </span>
                      </div>
                      <p className="font-mono text-xs text-text-muted">Formation: {userSquad.formation}</p>
                      <div className="pt-2 border-t border-border-muted flex items-center justify-between text-xs font-mono">
                        <span className="text-text-muted">Squad Chemistry</span>
                        <span className="text-accent font-bold">{userSquad.chemistry.overall}%</span>
                      </div>
                    </div>
                  ) : (
                    <div className="text-center py-6 space-y-3">
                      ...
                      <button
                        ...
                        className="px-4 py-2 bg-accent text-volt-text font-mono font-bold text-xs rounded-xl uppercase tracking-wider shadow-md"
                      >
                        AutoSquad Match
                      </button>
                    </div>
          ...
          {activeTab === 'Peak Stats' && (
            <div className="p-6 rounded-3xl bg-surface border border-border-muted space-y-6">
              <h2 className="font-sans font-bold text-base text-text-primary uppercase tracking-wider">Career Performance Trends</h2>
              <div className="h-64 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={matchPerformanceData}>
                    <XAxis dataKey="match" tick={{ fill: 'var(--text-muted)', fontSize: 11, fontFamily: 'Urbanist' }} axisLine={false} tickLine={false} />
                    <YAxis domain={[0, 10]} tick={{ fill: 'var(--text-muted)', fontSize: 11, fontFamily: 'Urbanist' }} axisLine={false} tickLine={false} />
                    <Tooltip contentStyle={{ background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: 8, color: 'var(--text-primary)' }} />
                    <Bar dataKey="rating" fill="var(--accent)" radius={[6, 6, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
          ...
          {activeTab === 'Glory Board' && (
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {[
                ...
                { title: 'Squad Leader', desc: 'Captained 10 victories', rarity: 'Obsidian', color: 'var(--accent)' },
                ...
                <div key={i} className="p-5 rounded-2xl bg-surface border border-border-muted space-y-2 text-center">
                  <div className="w-12 h-12 rounded-2xl mx-auto flex items-center justify-center bg-elevated border border-border-muted text-text-primary">
                    <Trophy size={22} style={{ color: b.color }} />
                  </div>
                  <p className="font-sans font-bold text-xs text-text-primary">{b.title}</p>
